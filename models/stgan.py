...
class ConvGRUCell(nn.Module):
    def __init__(self, n_attrs, in_dim, out_dim, in_state_dim, kernel_size=3):
        ...
        self.upsample = nn.Sequential(
            ...
            nn.Conv2d(in_state_dim + n_attrs, out_dim, bias=False, stride=1,
                      kernel_size=3),
...
def get_encoder_layers(conv_dim=64, n_layers=5, max_dim=1024, norm=nn.BatchNorm2d,
...
class Generator(nn.Module):
    def __init__(self, attr_dim, conv_dim=64, n_layers=5, max_dim=1024, shortcut_layers=2,
                 stu_kernel_size=3, use_stu=True, one_more_conv=True, n_attr_deconv=1):
        ...
        self.upsample = lambda x, y: nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(x, y, bias=False, stride=1, kernel_size=3, padding=1),
            # nn.BatchNorm2d(y),
            # nn.LeakyReLU(inplace=True)
        )
        ##### build encoder
        enc_layers = get_encoder_layers(conv_dim, n_layers, max_dim,
                                        bias=True)  # NOTE bias=false for STGAN
        ...
        for i in reversed(range(self.n_layers)):
            ...
            if i >= self.n_layers - 1 - self.shortcut_layers and i != self.n_layers - 1:  # skip connection
                ...
                if use_stu:
                    self.stu.append(ConvGRUCell(self.n_attrs, enc_size, enc_size,
                                                min(max_dim, enc_size * 2),
                                                stu_kernel_size))
...
class Latent_Discriminator(nn.Module):
    def __init__(self, image_size=128, max_dim=512, attr_dim=10, conv_dim=64, fc_dim=1024,
                 n_layers=5, shortcut_layers=2):
        super(Latent_Discriminator, self).__init__()
        layers = []
        n_dis_layers = int(np.log2(image_size))
        layers = get_encoder_layers(conv_dim, n_dis_layers, max_dim, norm=nn.BatchNorm2d,
...
class Discriminator(nn.Module):
    def __init__(self, image_size=128, max_dim=512, attr_dim=10, conv_dim=64, fc_dim=1024,
        ...
        layers = []
        layers = get_encoder_layers(conv_dim, n_layers, max_dim, norm=nn.InstanceNorm2d,
                                    bias=True)
...
if __name__ == '__main__':
    gen = Generator(5, conv_dim=32, n_layers=6, shortcut_layers=0, max_dim=512,
    ...
    dis = Latent_Discriminator(image_size=128, max_dim=512, attr_dim=5, conv_dim=32,
                               n_layers=6, shortcut_layers=2)
    print(dis)
