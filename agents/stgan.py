...
class STGANAgent(object):
    def __init__(self, config):
        ...
        self.G = Generator(len(self.config.attrs), self.config.g_conv_dim,
                           self.config.g_layers, self.config.max_conv_dim,
        ...
        self.D = Discriminator(self.config.image_size, self.config.max_conv_dim,
                               len(self.config.attrs), self.config.d_conv_dim,
                               self.config.d_fc_dim, self.config.d_layers)
        self.LD = Latent_Discriminator(self.config.image_size, self.config.max_conv_dim,
                                       len(self.config.attrs), self.config.d_conv_dim,
                                       self.config.d_fc_dim, self.config.g_layers,
    ...
    def save_checkpoint(self):
        def save_one_model(model, optimizer, name):
            ...
            torch.save(state, os.path.join(self.config.checkpoint_dir,
                                           '{}_{}.pth.tar'.format(name,
    ...
    def load_checkpoint(self):
        ...
        def load_one_model(model, optimizer, name):
            G_checkpoint = torch.load(os.path.join(self.config.checkpoint_dir,
                                                   '{}_{}.pth.tar'.format(name,
                                                                          self.config.checkpoint)),
                                      map_location=torch.device('cpu'))
            G_to_load = {k.replace('module.', ''): v for k, v in
                         G_checkpoint['state_dict'].items()}
            model.load_state_dict(G_to_load)
            model.to(self.device)
            if self.config.mode == 'train':
                optimizer.load_state_dict(G_checkpoint['optimizer'])

        load_one_model(self.G, self.optimizer_G if self.config.mode == 'train' else None,
                       'G')
        load_one_model(self.D, self.optimizer_D if self.config.mode == 'train' else None,
                       'D')
        load_one_model(self.LD,
                       self.optimizer_LD if self.config.mode == 'train' else None, 'LD')
    ...
    def compute_sample_grid(self, x_sample, c_sample_list, c_org_sample, path,
                            writer=False):
        ...
        for c_trg_sample in c_sample_list:
            ...
            for im in range(fake_image.shape[0]):
                ...
                for i in range(attr_diff.shape[1]):
                    cv2.putText(image, "%.2f" % (c_trg_sample[im][i].item()),
                                (10, 14 * (i + 1)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 255, 255, 255), 2, 8)
                    cv2.putText(image, "%.2f" % (out_att[im][i].item()),
                                (10, 14 * (i + 7)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 255, 255, 255), 2, 8)
                image = ((image.astype(np.float32)) / 255).transpose(2, 0, 1) + \
                        fake_image[im].cpu().detach().numpy()
                fake_image[im] = torch.from_numpy(
                    image)  # transforms.ToTensor()(image)*(2)-1

            x_fake_list.append(fake_image)
        x_concat = torch.cat(x_fake_list, dim=3)
        if writer:
            self.writer.add_image('sample',
                                  make_grid(self.denorm(x_concat.data.cpu()), nrow=1),
                                  self.current_iteration)
    ...
    def train(self):
        ...
        self.optimizer_D = optim.Adam(self.D.parameters(), self.config.d_lr,
                                      [self.config.beta1, self.config.beta2])
        self.optimizer_LD = optim.Adam(self.LD.parameters(), self.config.ld_lr,
                                       [self.config.beta1, self.config.beta2])
        ...
        self.lr_scheduler_D = optim.lr_scheduler.StepLR(self.optimizer_D,
                                                        step_size=self.config.lr_decay_iters,
                                                        gamma=0.1)
        self.lr_scheduler_LD = optim.lr_scheduler.StepLR(self.optimizer_LD,
                                                         step_size=self.config.lr_decay_iters,
                                                         gamma=0.1)
            ...
            for it in range(self.data_loader.train_iterations):
                ...
                if self.config.use_image_disc or self.config.use_classifier_generator:
                    ...
                    if self.config.use_image_disc:
                        ...
                        x_hat = (alpha * Ia.data + (
                                    1 - alpha) * Ib_hat.data).requires_grad_(True)
                ...
                if self.config.use_latent_disc:
                    ...
                    _, z = self.G(Ia,
                                  a_att_copy - a_att_copy if self.config.use_attr_diff else a_att_copy)
                    out_att = self.LD(z)

                    # classification loss
                    ld_loss = self.classification_loss(out_att,
                                                       a_att) * self.config.lambda_ld
                ...
                # =================================================================================== #
                if (
                        self.current_iteration + 1) % self.config.n_critic == 0:  # and i>20000:
                    self.G.train()
                    self.D.eval()
                    self.LD.eval()
                ...
                if self.current_iteration % self.config.summary_step == 0:
                    ...
                    print(
                        'Elapsed [{}], Iteration [{}/{}] Epoch [{}/{}] (Iteration {})'.format(
                            et, it, self.data_loader.train_iterations, batch,
                            self.config.max_epoch, self.current_iteration))
                ...
                if (self.current_iteration) % self.config.sample_step == 0:
                    self.G.eval()
                    with torch.no_grad():
                        self.compute_sample_grid(Ia_sample, b_samples, a_sample,
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(tqdm_loader):
                ...
                self.compute_sample_grid(x_real, c_trg_list, c_org,
                                         os.path.join(self.config.result_dir,
                                                      'sample_{}.jpg'.format(i + 1)),
                                         writer=False)
            ...
            for i, (x_real, c_org) in enumerate(tqdm_loader):
                ...
                for axis in range(10):
                    ...
                    samples = np.broadcast_to(
                        np.linspace(min_axis[axis] - margin_axis[axis],
                                    max_axis[axis] + margin_axis[axis], 10),
                        (n_images, 10)).transpose(1, 0)
                    ...
                    for sample in samples:
                        ...
                        tensor_codes = torch.from_numpy(
                            edited_embs.reshape(edited_embs.shape[0], 512, 2, 2).astype(
                                np.float32))
                        fake_image = self.G.decode(tensor_codes.to(self.device),
                                                   attr_diff.to(self.device))
                        # edited_embs = np.repeat(reducted_emb, 10, 0)
                        x_fake_list.append(fake_image)
                    x_concat = torch.cat(x_fake_list, dim=3)
                    save_image(self.denorm(x_concat.data.cpu()),
                               os.path.join(self.config.result_dir,
                                            'sample_ica_axis_{}_{}.jpg'.format(axis,
                                                                               i + 1)),
                               nrow=1, padding=0)
    ...
    def finalize(self):
        ...
        self.writer.export_scalars_to_json(
            os.path.join(self.config.summary_dir, 'all_scalars.json'))
        self.writer.close()
